...
// ==========================================
// CONFIGURAÇÕES VISUAIS DA LISTA (DARK MODE PREMIUM)
// ==========================================
const statusStyles: any = {
  aberto: { 
    label: "Aberto", 
    color: "text-blue-700 bg-blue-50 border-blue-200 dark:bg-blue-500/20 dark:text-blue-300 dark:border-blue-500/30",
    icon: Clock,
  },
  aprovado: { 
    label: "Aprovado", 
    color: "text-amber-700 bg-amber-50 border-amber-200 dark:bg-amber-500/20 dark:text-amber-300 dark:border-amber-500/30",
    icon: Package,
  },
  rejeitado: { 
    label: "Rejeitado", 
    color: "text-red-700 bg-red-50 border-red-200 dark:bg-red-500/20 dark:text-red-300 dark:border-red-500/30",
    icon: XCircle,
  },
  entregue: { 
    label: "Entregue", 
    color: "text-emerald-700 bg-emerald-50 border-emerald-200 dark:bg-emerald-500/20 dark:text-emerald-300 dark:border-emerald-500/30",
    icon: CheckCircle2,
  },
};
...
const EmptyState = ({ title, description }: { title: string; description: string }) => (
  <div className="flex flex-col items-center justify-center p-8 sm:p-10 text-center border border-dashed rounded-2xl sm:rounded-3xl border-muted bg-muted/20 dark:bg-muted/5 dark:border-muted/20 min-h-[250px] sm:min-h-[300px] animate-in fade-in zoom-in duration-300 w-full col-span-full mt-4">
    <div className="h-14 w-14 sm:h-20 sm:w-20 rounded-full bg-primary/10 dark:bg-primary/20 flex items-center justify-center mb-4 sm:mb-6">
      <Package className="h-7 w-7 sm:h-10 sm:w-10 text-primary/60 dark:text-primary/80" />
    </div>
           ...
           <div key={step.id} className="relative flex gap-5 sm:gap-6">
             ...
             {!isLast && (
               <div className={cn(
                 ...
                 lineCompleted && !step.isRejected ? "bg-emerald-500 dark:bg-emerald-500/80" : "bg-border dark:bg-border/50"
               )} />
             )}
             ...
             <div className="relative flex flex-col items-center z-10 pt-1 shrink-0">
               {step.isCompleted && !step.isActive && !step.isRejected ? (
                 <div className="h-6 w-6 rounded-full bg-emerald-500 flex items-center justify-center ring-4 ring-card dark:ring-background shadow-sm">
                   <Check className="h-3.5 w-3.5 text-white" strokeWidth={3} />
                 </div>
               ) : step.isActive && !step.isRejected ? (
                 <div className="relative h-6 w-6 rounded-full bg-card border-[3px] border-emerald-500 flex items-center justify-center ring-4 ring-card dark:ring-background shadow-sm">
                   <div className="h-2.5 w-2.5 rounded-full bg-emerald-500" />
                   <div className="absolute inset-[-4px] rounded-full border border-emerald-500/50 animate-ping" />
                 </div>
               ) : step.isRejected ? (
                 <div className="h-6 w-6 rounded-full bg-red-500 flex items-center justify-center ring-4 ring-card dark:ring-background shadow-sm">
                   <X className="h-3.5 w-3.5 text-white" strokeWidth={3} />
                 </div>
               ) : (
                 <div className="h-6 w-6 rounded-full bg-card dark:bg-background border-[3px] border-border dark:border-border/60 flex items-center justify-center ring-4 ring-card dark:ring-background" />
               )}
             </div>
...
export default function Requests() {
  ...
  // CORREÇÃO 1: Adicionados mais eventos de Socket e Refetch Duplo (Evita Race Condition no Banco)
  useEffect(() => {
    if (!socket) return;
    ...
    const handleUpdate = () => {
      ...
      // Invalida novamente 500ms depois para garantir que o BD concluiu a alteração
      setTimeout(() => {
        queryClient.invalidateQueries({ queryKey: ["requests"] });
      }, 500);
    };

    // Ouvimos todos os possíveis eventos que o backend pode emitir
    const events = ["new_request", "refresh_requests", "request_updated", "update_request", "status_updated"];
  ...
  // CORREÇÃO 2: Adicionado staleTime para a página não "pensar" se fores a outro separador e voltares
  const { data: requests, isLoading } = useQuery({
    queryKey: ["requests"],
    queryFn: async () => (await api.get("/requests")).data,
    refetchInterval: 15000, 
    staleTime: 1000 * 30, // Mantém o cache por 30 segundos
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      // CORREÇÃO 3: Refetch Duplo nas Mutações também
      queryClient.invalidateQueries({ queryKey: ["requests"] });
      setTimeout(() => queryClient.invalidateQueries({ queryKey: ["requests"] }), 500);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-6 space-y-5 sm:space-y-6 animate-in fade-in duration-500 pb-24 overflow-x-hidden min-h-screen">
      ...
      <div className="flex flex-col md:flex-row gap-3 sm:gap-4 justify-between items-start md:items-center bg-card dark:bg-card/40 dark:backdrop-blur-md p-2 sm:p-4 rounded-xl sm:rounded-[1.25rem] border border-border/50 dark:border-border/30 shadow-sm w-full min-w-0">
         <div className="relative w-full md:max-w-md group shrink-0">
            ...
            <Input 
                ...
                className="pl-9 sm:pl-10 h-9 sm:h-12 bg-muted/40 dark:bg-muted/10 border-transparent rounded-lg sm:rounded-xl focus:bg-background dark:focus:bg-background/40 focus:ring-2 focus:ring-primary/20 transition-all font-medium text-xs sm:text-base w-full"
                value={searchTerm}
         ...
         <div className="w-full overflow-x-auto scrollbar-hide pb-1 sm:pb-0">
            <div className="flex bg-muted/50 dark:bg-muted/10 p-1 rounded-lg sm:rounded-xl w-max min-w-full gap-1">
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                ...
                                isActive ? "bg-background text-foreground shadow-sm ring-1 ring-border/50 dark:ring-border/40" : "text-muted-foreground hover:text-foreground hover:bg-background/50 dark:hover:bg-background/10"
                            )}
                        >
            ...
            {filteredRequests.map((request: any) => {
               ...
               return (
                  <Card 
                     key={request.id} 
                     className={cn(
                         "group cursor-pointer rounded-xl sm:rounded-3xl border border-border/50 dark:border-border/30 bg-card dark:bg-card/40 hover:dark:bg-card/80 shadow-sm dark:shadow-none hover:shadow-md transition-all duration-200 flex flex-col justify-between overflow-hidden w-full min-w-0",
                         request.status === 'aberto' && "ring-1 sm:ring-2 ring-primary/30 dark:ring-primary/40 border-transparent bg-primary/[0.02] dark:bg-primary/[0.03]" 
                     )}
                  ...
                  >
                     <div className="p-3 sm:p-5 pb-2 sm:pb-3 flex flex-col min-w-0">
                         ...
                         <div className="w-full min-w-0">
                             ...
                             <div className="flex items-center gap-1 sm:gap-1.5 text-[11px] sm:text-sm font-semibold text-muted-foreground bg-background dark:bg-background/40 w-fit max-w-full px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md border border-border/50 dark:border-border/30">
                     ...
                     <div className="px-3 sm:px-5 py-2 sm:py-4 bg-muted/30 dark:bg-muted/10 border-t border-border/50 dark:border-border/30 flex items-center justify-between w-full min-w-0">
                        <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
                            <div className="h-6 w-6 sm:h-8 sm:w-8 rounded-full bg-background border shadow-sm dark:shadow-none flex items-center justify-center shrink-0">
                        ...
                        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                            {canManage && (
                                <Button 
                                    ...
                                    className="h-7 w-7 sm:h-9 sm:w-9 text-muted-foreground hover:bg-red-100 hover:text-red-600 dark:hover:bg-red-900/30 dark:hover:text-red-400 rounded-full z-10"
                                    onClick={(e) => openDeleteDialog(request.id, e)}
                                >
                                    <Trash2 className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                                </Button>
                            )}
                            <div className="h-7 w-7 sm:h-9 sm:w-9 rounded-full bg-foreground text-background dark:bg-primary dark:text-primary-foreground flex items-center justify-center group-hover:scale-110 transition-transform shadow-md dark:shadow-none">
      ...
      {/* ================================================================= */}
      {/* MODAL DETALHADO (DARK MODE PREMIUM) */}
      {/* ================================================================= */}
      <Dialog open={!!selectedRequest} onOpenChange={() => closeAllDialogs()}>
        <DialogContent className="w-[95vw] sm:w-full max-w-2xl bg-card dark:bg-background/95 dark:backdrop-blur-xl border-border/50 dark:border-border/30 p-0 overflow-hidden shadow-2xl dark:shadow-none rounded-[24px] sm:rounded-[32px] flex flex-col max-h-[90dvh]">
          
          {/* HEADER MERCADO LIVRE (Status Gigante) */}
          {selectedRequest && (
            <div className={cn(
              "px-5 sm:px-8 py-6 sm:py-8 border-b border-border/50 dark:border-border/20 flex flex-col gap-1 sm:gap-2 shrink-0 relative overflow-hidden",
              selectedRequest.status === 'entregue' ? "bg-emerald-50 dark:bg-emerald-500/15" :
              selectedRequest.status === 'rejeitado' ? "bg-red-50 dark:bg-red-500/15" :
              selectedRequest.status === 'aprovado' ? "bg-amber-50 dark:bg-amber-500/15" : "bg-blue-50 dark:bg-blue-500/15"
            )}>
                {/* Elemento gráfico de fundo */}
                <div className="absolute -right-10 -top-10 opacity-10 dark:opacity-[0.08] pointer-events-none">
                ...
                <p className="text-sm sm:text-base font-medium text-foreground/80 dark:text-foreground/90 z-10">
          ...
          <div className="flex-1 overflow-y-auto p-0 bg-background dark:bg-transparent">
            
            {/* Bloco de Rastreio (Timeline) */}
            {selectedRequest && (
                <div className="p-5 sm:p-8 border-b border-border/50 dark:border-border/30">
                    <MLTimeline request={selectedRequest} />
                </div>
            )}

            {/* Bloco de Destinatário / Setor */}
            <div className="p-5 sm:p-8 border-b border-border/50 dark:border-border/30 flex gap-4 items-start">
                <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-muted dark:bg-muted/30 flex items-center justify-center text-muted-foreground shrink-0 mt-1">
            ...
            <div className="p-5 sm:p-8">
              ...
              <div className="border border-border/60 dark:border-border/30 rounded-xl sm:rounded-2xl overflow-hidden bg-card dark:bg-card/20 shadow-sm dark:shadow-none">
                <div className="divide-y divide-border/60 dark:divide-border/30">
                  {selectedRequest?.request_items?.map((item: any, i: number) => (
                    <div key={i} className="flex items-center justify-between p-4 sm:p-5 hover:bg-muted/10 dark:hover:bg-muted/5 transition-colors w-full min-w-0">
                        <div className="flex flex-col pr-3 sm:pr-4 min-w-0 flex-1">
                            ...
                            {item.products?.sku && (
                                <span className="font-mono text-[10px] sm:text-xs text-muted-foreground mt-1 bg-muted dark:bg-muted/30 w-fit px-1.5 py-0.5 rounded">
                        ...
                        <div className="flex flex-col items-end shrink-0">
                            ...
                            <span className="text-base sm:text-lg font-black text-foreground bg-muted/50 dark:bg-muted/30 px-2 py-0.5 rounded-lg border border-border/50 dark:border-border/30">
          ...
          <div className="p-4 sm:p-6 bg-card dark:bg-transparent border-t border-border/50 dark:border-border/30 flex flex-col sm:flex-row gap-2 sm:gap-3 shrink-0">
            {canManage && selectedRequest?.status === 'aberto' ? (
              <>
                ...
                <Button 
                  className="flex-[2] rounded-xl h-12 sm:h-14 bg-emerald-600 dark:bg-emerald-500 hover:bg-emerald-700 dark:hover:bg-emerald-600 text-white shadow-md sm:shadow-lg shadow-emerald-600/20 dark:shadow-none font-black text-sm sm:text-lg min-w-0 truncate" 
              ...
              <Button 
                  className="w-full rounded-xl h-12 sm:h-14 bg-blue-600 hover:bg-blue-700 text-white shadow-md sm:shadow-lg shadow-blue-600/20 dark:shadow-none font-black text-sm sm:text-lg min-w-0 truncate" 
                  onClick={() => handleDeliver(selectedRequest.id)}
                >
                  <Truck className="mr-1.5 sm:mr-2 h-4 w-4 sm:h-6 sm:w-6 shrink-0" /> Confirmar Entrega
              </Button>
            ) : (
              <Button variant="secondary" className="w-full rounded-xl h-12 sm:h-14 font-bold text-sm sm:text-base bg-muted/80 dark:bg-muted/20 hover:bg-muted dark:hover:bg-muted/40 text-foreground min-w-0 truncate" onClick={closeAllDialogs}>
      ...
      <Dialog open={isRejectDialogOpen} onOpenChange={setIsRejectDialogOpen}>
        <DialogContent className="w-[95vw] bg-card dark:bg-background/95 dark:backdrop-blur-xl border-border dark:border-border/30 rounded-[24px] sm:rounded-[32px] p-5 sm:p-6 sm:max-w-md">
          ...
          <Textarea 
            ...
            className="bg-background dark:bg-background/50 min-h-[100px] sm:min-h-[120px] rounded-xl border-border/50 dark:border-border/30 focus:ring-2 focus:ring-red-500/20 text-sm sm:text-base p-3 sm:p-4 resize-none shadow-inner dark:shadow-none"
          />
          <DialogFooter className="mt-4 sm:mt-6 flex flex-row gap-2 sm:gap-3">
            <Button variant="outline" className="flex-1 rounded-xl h-10 sm:h-12 font-bold text-xs sm:text-sm dark:border-border/50" onClick={() => setIsRejectDialogOpen(false)}>Voltar</Button>
            <Button variant="destructive" className="flex-1 rounded-xl h-10 sm:h-12 font-bold text-xs sm:text-sm shadow-md shadow-red-500/20 dark:shadow-none" onClick={() => requestActionId && updateStatusMutation.mutate({ id: requestActionId, status: "rejeitado", reason: rejectionReason })}>
              Confirmar
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="w-[95vw] bg-card dark:bg-background/95 dark:backdrop-blur-xl border-border dark:border-border/30 rounded-[24px] sm:rounded-[32px] p-5 sm:p-6 sm:max-w-md">
          ...
          <DialogFooter className="flex flex-row gap-2 sm:gap-3 w-full">
            <Button variant="outline" className="flex-1 rounded-xl h-10 sm:h-12 font-bold text-xs sm:text-sm dark:border-border/50" onClick={() => setDeleteDialogOpen(false)}>Cancelar</Button>
            <Button variant="destructive" className="flex-1 rounded-xl h-10 sm:h-12 font-bold text-xs sm:text-sm shadow-md shadow-red-500/20 dark:shadow-none" onClick={() => requestActionId && deleteRequestMutation.mutate(requestActionId)}>
                Sim, Excluir
            </Button>
          </DialogFooter>
        </DialogContent>
