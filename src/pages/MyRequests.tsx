...
export default function MyRequests() {
  ...
  // 🔥 CORREÇÃO: Lógica do Estoque Disponível atualizada!
  const getAvailableStock = (product: any) => {
    const stockInfo = product.stock; 
    if (!stockInfo) return 0;

    // 1. Prioridade: Se o backend já retorna "quantity_available", usamos ele.
    if (stockInfo.quantity_available !== undefined) {
      return Number(stockInfo.quantity_available);
    }
    
    // 2. Caso contrário, calculamos Físico - Reservado
    ...
    // O valor disponível nunca pode ser negativo
