...
interface TransactionPanelProps {
  ...
  opCode: string; // <-- ADICIONADO
  setOpCode: (op: string) => void; // <-- ADICIONADO
}
...
export function TransactionPanel({
  ...
  isPending, resetTransaction, opCode, setOpCode // <-- ADICIONADO AQUI
        ...
        <Card className="lg:col-span-3 flex flex-col h-[400px] lg:h-[calc(100vh-10rem)] border-muted-foreground/20 shadow-sm overflow-hidden order-1">
          ...
          <CardContent className="flex-1 overflow-y-auto p-2 space-y-2">
            {filteredStocks.map((stock: StockItem) => {
               // 🚀 CÁLCULO PADRONIZADO: Subtraindo o reservado do físico com fallback seguro
               const physicalStock = Number(stock.quantity_on_hand) || 0;
               const reservedStock = Number(stock.quantity_reserved) || 0;
                ...
                >
                  ...
                  <div className="flex justify-between items-center mt-2 text-xs text-muted-foreground">
                    ...
                    <span className={isEntry ? "" : (available > 0 ? "text-emerald-600 font-bold" : "text-red-500 font-bold")}>
                      {isEntry ? physicalStock : available} {stock.products?.unit}
                    </span>
                  </div>
                </div>
               );
            ...
            {!isEntry && (
              <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                ...
                <div className="w-full sm:w-48">
                  <Input 
                    placeholder="Nº da OP (se houver)"
                    value={opCode}
                    onChange={(e) => setOpCode(e.target.value)}
                    className="h-10 border-red-200 bg-red-50/50 focus-visible:ring-red-500/30"
                  />
                </div>
              </div>
            )}
